...
output_path = pathlib.Path(
    "/glade/scratch/arezoo/PR/forcing")
...
file_chunked = output_path / "forcing.zarr"
...
coord_file = '/glade/p/cisl/nwc/nwmv30_finals/PuertoRico/DOMAIN_WCOSS_Names/GEOGRID_LDASOUT_Spatial_Metadata_PRVI.nc'

# input_dir = "/glade/campaign/ral/hap/zhangyx/AORC.Forcing"
input_dir = '/glade/scratch/arezoo/pr_precip/'
...
def main():
    ...
    files = [
        pathlib.Path(
            f"{input_dir}/"
            f'{date.strftime("%Y%m%d%H")}.LDASIN_DOMAIN1'
        )
        for date in dates
    ]
    ...
    #    print(files_chunk)
        ...
        ds['x'] = dset['x']
        ds['y'] = dset['y'][::-1] # since getting the y from the coord system, we will reverse it
