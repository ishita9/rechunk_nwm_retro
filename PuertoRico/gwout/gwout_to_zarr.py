...
# User config
output_path = pathlib.Path("/glade/scratch/arezoo/PR/gwout/")
...
input_dir = "/glade/p/cisl/nwc/nwmv30_finals/PuertoRico/retro_final/OUTPUT_GW/"
...
def main():
    ...
    files = [
        pathlib.Path(
            f"{input_dir}/"
            f'{date.strftime("%Y%m%d%H%M")}.GWOUT_DOMAIN1'
        )
        for date in dates
    ]
