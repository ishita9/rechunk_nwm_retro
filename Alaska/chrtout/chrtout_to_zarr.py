...
# User config
output_path = pathlib.Path("/glade/derecho/scratch/ishitas/Alaska/chrtout")
...
end_date = '2019-12-31 23:00'  # full time
#end_date = "1980-02-01 00:00"  # pilot 2.5 months
...
input_dir = "/glade/p/cisl/nwc/nwmv30_finals/Alaska/retro/retro40yr/outputs_by_year"
start_date = "1981-01-01 01:00"

freq = "1h"

drop_vars = ["reference_time"]
...
def get_gage_id():
    chrtout_file = pathlib.Path(
        '/glade/p/cisl/nwc/nwmv30_finals/Alaska/retro/retro40yr/outputs_by_year/'
        '1981/198102020000.CHRTOUT_DOMAIN1')
    rl_file = pathlib.Path(
        '/glade/p/cisl/nwc/nwmv30_finals/Alaska/retro/retro40yr/run.retro/DOMAIN/'
        'RouteLink_Alaska_NWMv3.0.nc')
...
def main():
    ...
    if file_chunked.exists():
        ...
        print("Last time in existing chunked block")
        print(last_time)
        ds.close()
        del ds
        dates = pd.date_range(
        ...
        print("Dates in existing chunk block")
        print(dates)
    else:
        dates = pd.date_range(
    ...
    files = [
        pathlib.Path(
            f"{input_dir}/"
            f'{date.strftime("%Y")}/'
            f'{date.strftime("%Y%m%d%H%M")}.CHRTOUT_DOMAIN1'
        )
        for date in dates
    ]
