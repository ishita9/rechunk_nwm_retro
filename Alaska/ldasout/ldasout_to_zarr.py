...
# User config
output_path = pathlib.Path("/glade/derecho/scratch/ishitas/Alaska/ldasout/")
...
def main():
    ...
    for ii in range(n_chunks_job_actual):
        ...
        # Modified by KMS 9/1/2023
        if not file_chunked.exists():
            ds["crs"] = xr.DataArray(np.array(b'', dtype='|S1'), name="crs", attrs=dset["crs"].attrs)
