...
# User config
output_path = pathlib.Path("/glade/derecho/scratch/ishitas/CONUS/rtout")
...
n_cores = 1
queue = "casper"
cluster_mem_gb = 35  ## 25 works for time 224

n_chunks_job = 1  # how many to do before exiting, 12 is approx yearly
...
start_date = "2023-01-04 03:00"
#start_date = "2022-12-01 03:00"
freq = "3h"
...
def main():
    ...
    files = [
        pathlib.Path(
            f"{input_dir}/"
            f'{date.strftime("%Y")}/'
            f'{date.strftime("%Y%m%d%H%M")}.RTOUT_DOMAIN1'
        )
        for date in dates
    ]
    ...
    for ii in range(n_chunks_job_actual):
        ...
        if len(files_chunk) == time_chunk_size:
            ...
            ds1 = ds.chunk(
                {"y": y_chunk_size, "x": x_chunk_size, "time": time_chunk_size})
